...
use App\Models\Tenant\{BlendOrder, Order};

class BlendOrderService
{
	public function addNewBlendOrder($request)
	{
		DB::beginTransaction();
		try {
			...
			if ($request->connected_order_id) {
				...
				if ($connected_order) {
					if (customerId($request->customer_id) !== $connected_order->customer->id || $connected_order->type == "blend" || $connected_order->type == "receiving") {
						return json_response(403, __("Tenant.invalid_connected_order"));
					}
					$connected_order_id = $connected_order->id;
				}
			}
			$order = Order::createOrder($request->all(), $type = "blend", $connected_order_id);
			
			BlendOrder::create(array_merge([
